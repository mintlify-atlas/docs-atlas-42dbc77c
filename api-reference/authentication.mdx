...
<Tabs>
  ...
  <Tab title="PUBLIC">
    `PUBLIC` keys exist in the `ApiPermission` enum and are tracked by the rate limiter, but **every protected route requires `ALL` permission**. The `"protected"` Ktor auth scheme rejects any key whose permission is not `ALL`. As a result, `PUBLIC` keys cannot reach any route beyond the open `/ping` and `/metrics` endpoints.

    `PUBLIC` keys are subject to rate limiting: **100 requests per 60-second sliding window** per key. Reserve this permission level for future use or for external tools that only need the open health-check and metrics endpoints.
  </Tab>
</Tabs>
