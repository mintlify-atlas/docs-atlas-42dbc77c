...
When `auto-connect.enabled` is `true`, the proxy intercepts the `PostLoginEvent` (Velocity) or `LoginEvent` (BungeeCord) and selects a destination server from all `ONLINE` instances whose `configurationName` matches `configuration-type`.
