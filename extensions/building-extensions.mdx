---
title: "Building Custom Extensions for Universe"
...
<Accordion title="RuntimeProvider — custom execution environments">
  ...
  ```kotlin
  // Skeletal implementation — see runtime-docker or runtime-k8s for full examples
  class MyRuntimeProvider : RuntimeProvider {
      override val runtimeKey = "my-runtime"

      override fun start(instance: Instance, config: Configuration) { /* ... */ }
      override fun stop(instance: Instance) { /* ... */ }
      override fun execute(instance: Instance, command: String) { /* ... */ }
  }
  ```
